...
module "imm_domain_9" {                                                        # <-- change when copying
  source = "github.com/bywhite/imm-iac-az-mods//imm-domain-6536-mod"  #?ref=v0.1.1"
...
# -----------------------------------------------------------------------------
  ...
  policy_prefix = "${local.az_policy_prefix}-domain09"                        # <-- change when copying
  ...
  tags = [
    ...
    { "key" : "domain", "value" : "${local.az_policy_prefix}-domain09" }      # <-- change when copying
  ]
...
# -----------------------------------------------------------------------------
 # Uplink VLANs Allowed List    Example: "5,6,7,8,100-130,998-1011"
  switch_vlans_6536 = local.az_vlans
