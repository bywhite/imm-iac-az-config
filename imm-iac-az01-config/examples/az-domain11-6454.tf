# # =============================================================================
# # This defines the desired configuration of the ofl-dev-az1-bml-1 IMM domain
# # 
...
module "imm_domain_11" {                                                        # <-- change when copying
  ...
  source = "github.com/bywhite/imm-iac-az-mods/imm-domain-6454-mod" #?ref=v0.0.1"
...
# -----------------------------------------------------------------------------
  ...
  policy_prefix = "${local.az_policy_prefix}-domain11"                        # <-- change when copying
  description   = "built by Terraform for ${local.az_policy_prefix}"

  #Every object created in the domain will have these tags
  tags = [
    { "key" : "environment", "value" : "dev" },
    { "key" : "orchestrator", "value" : "Terraform" },
    { "key" : "az", "value" : "${local.az_policy_prefix}" },
    { "key" : "domain", "value" : "${local.az_policy_prefix}-domain11" } #    <-- change when copying
  ]
...
# -----------------------------------------------------------------------------
  ...
  switch_vlans_6454 = local.az_vlans
